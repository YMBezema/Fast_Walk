...
//	keySprint		= PAD::IS_CONTROL_PRESSED(0, MISC::GET_HASH_KEY("INPUT_SPRINT"));
	...
	if (moveBlendRatio >= 1.f && lsMagnitude >= 5)
	{
		...
		else
		{
			...
			temp = 1 + ((u - 1) * (lsMagnitude / 127.f)); //set moveblend speed to this value
			u = BUILTIN::CEIL(temp * 4.f) / 4.f; //but while holding sprint, save this value
		}

		target_moveBlendRatio = u; //this is retarded right?
